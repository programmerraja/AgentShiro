...
def parse_inline_tools(content: str, available_tools: list = None):
    ...
    if available_tools:
        ...
        # If explicitly passed an empty tools list, no tools are valid!
        return []
    ...
    for match in matches:
        ...
        # Guard: Only parse if it's an actual defined tool
...
class ShiroCustomProvider(BaseCustomLLM):
    ...
    def completion(self, model: str, messages: list, **kwargs) -> ModelResponse:
        client = OpenAI(api_key="dummy", base_url="http://localhost:8080")
        
        # Safely extract tools if they exist
        ...
        def stream_generator():
            ...
            for chunk in completion_resp:
                if chunk.choices and getattr(chunk.choices[0].delta, "content", None):
                    ...
                    content_buffer += text
                    # Yield simple Dict matching GenericStreamingChunk
                    yield {
                        "text": text,
                        "is_finished": False,
                        "finish_reason": "",
            ...
            if tool_calls:
                ...
                # Final chunk to signal stop and tool_calls finish reason
                yield {
                    "text": "",
                    "is_finished": True,
